    ...
    */
    ...
    for (j = 0; j < tc->dev_profile->n_hunks; j++) {
        ...
        case 1:
            ...
            break;
        case 4:
            ALOGI("vstep file hunk %d at %d", j, hunk->offset);
            break;
        case 5:
...
{
printf("CRP %s n_payload = %d blob_num %d cmd %x %x %x\n", __func__, n_payload, blob->blob_num, blob->cmd[0], blob->cmd[1], blob->cmd[2]);
    ...
    if (!tfa_get_bitfield(t, BF_COOLFLUX_CONFIGURED)) {
        if (!blob->blob_num) {
            if (blob->cmd[2]) {
                ...
                cmd[1] = blob->cmd[1];
                cmd[2] = blob->blob_num;
                return tfa_dsp_msg_id(t, 3*(n_payload-1), blob->data, cmd_bytes_to_cmd(cmd));
            } else {
                return tfa_dsp_msg(t, 3*n_payload, (unsigned char *) &blob->cmd[0]);
            }
...
ALOGI("n_blobs = %d at %d", n_blobs, (int) (ptr - tc->raw));
    ...
    n_regs = *regs_ptr++;
printf("CRP writing %d volume file registers\n", n_regs);
...
static int write_patch_file(tfa_t *t, patch_t *patch)
{
    return tfa_dsp_patch(t, patch->hdr.size - sizeof(patch->hdr), patch->data);
}

static int write_speaker_parameters(tfa_t *t, unsigned n_bytes, unsigned char *ptr)
{
    unsigned zero = 0;

printf("CRP %s %d bytes of speaker parameters\n", __func__, n_bytes);
...
static int write_file(tfa_cont_t *tc, unsigned char *ptr, int vstep, int a4, tfa_t *t)
{
    ...
    switch(h->magic) {
    case 0x4150:
        return write_patch_file(t, (patch_t *) &ptr[8]);
    case 0x5053:
        return write_speaker_parameters(t, h->size - 79, &ptr[81+6]);
    case 0x5056:
...
static int write_item(tfa_cont_t *tc, hunk_t *hunk, int vstep, tfa_t *t)
{
    ...
    default:
        ALOGE("Unsupported item type %d", hunk->type);
        break;
    case 4:
    case 5:
...
//printf("CRP write item %d type %d\n", i, profile->hunk[i].type);
                write_item(tc, &profile->hunk[i], vstep, t);
wrote_it = 1;
            }
        }
//if (! wrote_it) printf("CRP skip  item %d type %d\n", i, profile->hunk[i].type);
    }
...
printf("CRP %s %d profile %d\n", __func__, __LINE__, profile_num);
    for (i = 0; i < p->n_hunks; i++) {
        unsigned char *ptr = &tc->raw[p->hunk[i].offset];

printf("CRP %s %d type %d\n", __func__, i, p->hunk[i].type);
...
printf("CRP %s %d profile_num %d\n", __func__, __LINE__, profile_num);
// TODO: This should call write_items
    for (i = 0; i < p->n_hunks && p->hunk[i].type != 17; i++) {
        if (p->hunk[i].type == 16) {
            write_item(tc, &p->hunk[i], 0, t);
        } else if (p->hunk[i].type == 2) {
            printf("CRP %s type 2 is %x\n", __func__, p->hunk[i].offset);
        }
    }
printf("CRP %s %d\n", __func__, __LINE__);
    return 0;
...
printf("CRP %s %d profile_num %d swprof_num %d\n", __func__, __LINE__, prof_num, tfa_get_swprof(t));
...
printf("CRP swprof->group %d profile->group %d\n", swprof->group, profile->group);
