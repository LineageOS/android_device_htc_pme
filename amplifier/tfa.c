...
static int wait_calibration(tfa_t *t)
{
    ...
    if (tfa_get_bitfield(t, BF_CALIBRATION_ONETIME)) {
        ...
        if (retry >= 50) {
            printf("CRP Timeout waiting for mtp to stop being busy\n");
            return -EBUSY;
        }
        ...
        if (retry >= 25) {
            printf("CRP Timeout waiting for calibration to complete\n");
            return -EBUSY;
        }
    } else {
        int calibrated = 0;
        ...
        if (!calibrated) {
            printf("CRP Timeout waiting for dsp to report calibrated\n");
        }
    }
...
int tfa_start(tfa_t *t, tfa_cont_t *tc, int vstep)
{
    int cal_profile = -1;
...
printf("CRP %s %d\n", __func__, __LINE__);
    ...
    if (cal_profile >= 0) {
        tfa_cont_write_profile(tc, cal_profile, 0, t);
    }

    if (strstr(tfa_cont_get_profile_name(tc, cal_profile), ".standby")) {
        tfa_power_off(t);
    } else {
        tfa_power_on(t);
    }
...
#if TODO
    // tfa_cont_search_for_filter_keyword(tc, cal_profile);
    if (tfa_cont_get_current_vstep(tc) != vstep) {
        tfa_cont_write_vstep_files(tc, cal_profile, vstep, t);
    }
