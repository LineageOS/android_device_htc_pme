...
static int cold_startup(tfa_t *t, tfa_cont_t *tc, int profile_num)
{
printf("CRP %s %d\n", __func__, __LINE__);
    tfa_startup(t, tc, profile_num);
    cold_boot(t, 1);
...
{
printf("CRP %s %d force %d profile_num %d\n", __func__, __LINE__, force_startup, profile_num);
...
printf("CRP consider enabling coolflux\n");
        if (tfa_get_bitfield(t, BF_ENBL_COOLFLUX)) {
