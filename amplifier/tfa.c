...
#define MAX_BYTES_TO_READ       255     /* TODO: Find this in the code! */

int tfa_dsp_read_mem(tfa_t *t, unsigned dmem_and_madd, int n_words, int *data)
{
    int dmem;
    unsigned char tmp[MAX_BYTES_TO_READ];
    int n_read = 0;
    ...
    while (n_read < n_words) {
        int this_n_read = MAX_BYTES_TO_READ / 3;
...
printf("n_data %d hunk_n %d\n", n_data, hunk_n);
printf("WR");
dump_raw(data, hunk_n);
printf("\n");
        write(t->fd, data, hunk_n);
